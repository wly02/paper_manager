...
权重增强：对 VQA 主题增加 1.2 倍权重（适配多模态 VQA 论文的分类精准度）；
阈值筛选：保留相似度≥0.4的主题，无匹配时兜底返回相似度最高的主题；
输出：按相似度降序返回匹配的主题列表。
...
精准筛：对同大小文件计算 MD5 哈希，哈希相同判定为重复；
保留规则：优先保留「创建时间最早、路径层级最少、文件名无后缀（如_1/_2）」的文件；
安全机制：默认预览模式（仅展示不删除），需--force参数才执行删除。
